...
public class BahmniDiagnosisAnswerConceptSaveCommandImplTest {
    ...
    public void shouldSaveNewDiagnosisAnswerConceptAndAddToUnclassifiedSetWhenConceptSourceAndReferenceCodeProvided() throws Exception {
        ...
        BahmniEncounterTransaction bahmniEncounterTransaction = getBahmniEncounterTransaction("CS Code", true);
        when(administrationService.getGlobalProperty(GP_DEFAULT_CONCEPT_SET_FOR_DIAGNOSIS_CONCEPT_UUID)).thenReturn("unclassified-concept-set-uuid");
        when(conceptService.getAllConceptSources(false)).thenReturn(getMockedConceptSources("CS name", "CS code"));
        when(conceptService.getConceptByUuid("unclassified-concept-set-uuid")).thenReturn(unclassifiedConceptSet);
        ...
        assertEquals("malaria-uuid",bahmniEncounterTransaction.getBahmniDiagnoses().get(0).getCodedAnswer().getUuid());
    }
    ...
    public void shouldNotCreateDiagnosisAnswerConceptWhenExistingConceptProvided() throws Exception {
        Concept newDiagnosisConcept = getDiagnosisConcept();
        Concept unclassifiedConceptSet = getUnclassifiedConceptSet();
        BahmniEncounterTransaction bahmniEncounterTransaction = getBahmniEncounterTransaction("CS Code", false);
        when(administrationService.getGlobalProperty(GP_DEFAULT_CONCEPT_SET_FOR_DIAGNOSIS_CONCEPT_UUID)).thenReturn("unclassified-concept-set-uuid");
        when(conceptService.getAllConceptSources(false)).thenReturn(getMockedConceptSources("CS name", "CS code"));
        when(conceptService.getConceptByUuid("unclassified-concept-set-uuid")).thenReturn(unclassifiedConceptSet);
        when(terminologyLookupService.getConcept(anyString(), anyString())).thenReturn(newDiagnosisConcept);
